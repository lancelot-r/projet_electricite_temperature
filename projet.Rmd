...
acf(arima_model_2$residuals)
pacf(arima_model_2$residuals)
```
```{r}
...
plot(fitted_values, fit$residuals, main="Résidus vs Valeurs Ajustées", 
     xlab="Valeurs Ajustées", ylab="Résidus")
abline(h=0, col="red")

hist(fit$residuals, breaks = 30, main = "Histogramme des résidus", probability = TRUE)
lines(density(fit$residuals), col = "blue")
lines(density(rnorm(1000000)), col = "red")

qqnorm(fit$residuals, plot.it = T)
...
```
```{r}
test_model <- auto.arima(ts_temp)
```

```{r}
fit <- arima(ts_temp, order = c(1,0,2), seasonal = list(order = c(0,0,2), period = 24))
...
plot(predict, xlim = c(63, 64), ylim = c(10, 40))
...
```
```{r}
test_predict <- forecast(test_model, h = 24)

par(mfrow = c(1, 2))
xlim_start <- as.POSIXct("2017-09-01 00:00:00")
xlim_end <- as.POSIXct("2017-09-01 23:00:00")

plot(test_predict, xlim = c(63, 64), ylim = c(10, 40))
plot(data$Datetime, data$Temperature, col = "red", type = "l", 
     xlim = c(xlim_start, xlim_end),
     xlab = "Temps", ylab = "Consommation électrique", 
     main = "Temperature", ylim = c(10, 40))
