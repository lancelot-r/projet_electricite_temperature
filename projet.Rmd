...
```{r}
st_temp_janv=ts(df_janvier$Temperature, frequency = 24, start = c(1,1))
...
```{r}
t<-seq(1,18-1/24,1/24)
df2=as.data.frame(c(trend_train))
df2=cbind(df2,t)
 lm1<-lm(df2$`c(trend_train)`~t) #ajustementlinéaire
 lm2<-lm(df2$`c(trend_train)`~t+I(t^2)) #ajustementquadratique
 lm3<-lm(df2$`c(trend_train)`~t+I(t^2)+I(t^3)) #ajustementpolydegré3
 #les R^2:
...
```
```{r}
plot(trend_train, type = "l", col = "black", pch = 16, main = "Série trend_train avec courbes de régression", 
...
#pred2 <- ts(predict(lm2), frequency = 24, start = c(18,1))
